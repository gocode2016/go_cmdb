...
class RemoteRedis:
    ...
    def __init__(self):
        self.__conn = redis.Redis(host='169.46.245.112', port=6380)
        self.chan_pub = "product_foreign"
        self.chan_sub = "product_foreign"
...
class LocalRedis:
    ...
    key = "%s-product_foreign" % time.strftime("%Y%m%d", time.localtime())
    ...
    def _save(self):
        self.__conn.slaveof(host='169.46.245.112', port=6380)
    ...
    def get(self):
        if self._save():
            self.log.info(u"服务端同步国外正式环境redis成功")
            ...
            self.log.error(u"服务端同步国外正式环境redis失败")
...
class MySQLClient:
    ...
    def insert(self):
        if self.new_asset_list:
            for asset in self.new_asset_list:
                ...
                data = (v.get("Hostname"),v.get("InnerIp"),v.get("OuterIp"),v.get("Os"),v.get("OsRelease"),v.get("Cpu"),v.get("Memory"),disk,v.get("Sn"),v.get("AssetType"),v.get("AssetEnvironment"),v.get("Manufacturer"),v.get("Model"),v.get("AssetStatus"),int(v.get("Idc")),insert_time,insert_time)
    ...
    def update(self):
        if self.update_asset_dict:
            for k,v in self.update_asset_dict.items():
                ...
                # if self.update_asset_dict[k].get("disk"):
                #     disk = ""
                #     for k1, v1 in self.update_asset_dict[k].get("disk").get("newVal").items():
                #         disk += "%s:%s " % (k1, v1)
                #     sql += "disk = '%s'," % disk
                #     del self.update_asset_dict[k]["disk"]
                ...
                sql += ",update_time='%s' where outer_ip='%s';" % (update_time,k)
    ...
    def select(self):
        sql = "select * from asset where asset_environment = 'product-foreign' or asset_environment = 'beta'"
        self.__cursor.execute(sql)
        for row in self.__cursor.fetchall():
            ...
            Model = row[13]
            AssetStatus = row[18]
            self.asset_list.append(OuterIp)
            self.asset_dict[OuterIp] = {"Hostname":Hostname,"Os":Os,"OsRelease":OsRelease,"Cpu":Cpu,"Memory":Memory,"Disk":Disk,
...
def main():
    log_file = os.path.dirname(os.getcwd())+"/logs/product_foreign.log"
    log_obj = log.init_logger(log_file)
    obj = RemoteRedis()
    redis_sub = obj.subscribe()
    while True:
        msg = redis_sub.parse_response()
        if msg[2] == b'product_foreign finish':
            log_obj.info(u"服务端已经收到国外正式环境发布的消息")
