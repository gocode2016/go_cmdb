...
class RemoteRedis:
    ...
    def __init__(self):
        self.__conn = redis.Redis(host='172.31.3.61', port=6379)
        self.chan_pub = "product_cn"
        self.chan_sub = "product_cn"
...
class LocalRedis:
    ...
    key = "%s-product_cn" % time.strftime("%Y%m%d", time.localtime())
    ...
    def _save(self):
        self.__conn.slaveof(host="172.31.3.61",port=6379)
    ...
    def get(self):
        if self._save():
            self.log.info(u"服务端同步国内正式环境redis成功")
            ...
            self.log.error(u"服务端同步国内正式环境redis失败")
...
class MySQLClient:
    ...
    def update(self):
        if self.update_asset_dict:
            for k,v in self.update_asset_dict.items():
                ...
                #     #     disk += "%s:%s " % (k1, v1)
                #     sql += "disk = '%s'," % self.update_asset_dict[k].get("disk").get("newVal")
    ...
    def select(self):
        sql = "select * from asset where asset_environment = 'product-cn'"
        self.__cursor.execute(sql)
...
def main():
    log_file = os.path.dirname(os.getcwd())+"/logs/product_cn.log"
    log_obj = log.init_logger(log_file)
    obj = RemoteRedis()
    redis_sub = obj.subscribe()
    while True:
        msg = redis_sub.parse_response()
        if msg[2] == b'product_cn finish':
            log_obj.info(u"服务端已经收到国内正式环境发布的消息")
